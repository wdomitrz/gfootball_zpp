...
def make_summary(in_path, out_path, depth, metric_files, tf_extract):
    ...
    def dfs(cur_path, prev_dirs, cur_depth):
        if cur_depth == depth:
            ...
            if os.path.isdir(metrics_dir):
                ...
                print("WARNING dir {} does not exists (no tf metrics?)".format(metrics_dir))
    ...
    def write_summary():
        ...
        for name, df in summary_dict.items():
            df.to_csv(os.path.join(out_path, name), sep=';', encoding='utf-8', index=False)
...
def main(argv):
    make_summary(FLAGS.in_path, FLAGS.out, FLAGS.level, FLAGS.metric_files.split(','), FLAGS.extract)
