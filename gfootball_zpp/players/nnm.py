...
def get_mostly_latest_model_path(path):
    ...
    if models_paths is None:
        return None
    sample_id = max(0, round((1 - abs(random.normal())) * (len(models_paths) - 1)))
...
class Player(player_base.PlayerBase):
    ...
    def __init__(self, player_config, env_config):
        ...
        self._model_reload_rate = int(player_config.get('model_reload_rate', 0))

        if 'models_dirs' not in player_config:
            raise Exception('wrong config for nnm player')
        
        models_dirs_spec = list(map(lambda x: x.split(';'), player_config['models_dirs'].split('*')))    
    ...
    def take_action(self, observation):
        ...
        if self._num_rewards is None:
            ...
            _, rewards, _, _ = self._converter.step([0] * (self._left_players + self._right_players))
        ...
        prev_actions, env_output, core_state = pack_nnm_input(self._left_players + self._right_players, self._num_rewards, observation, self._core_state)
